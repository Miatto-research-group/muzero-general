...
def make_random_unitary(qbg1=[], qbg2=[], nb_steps:int=3, size:int=3):
    ...
    for _ in range(nb_steps):
        dice_roll = np.random.randint(1,3) #only 1 and 2
        if dice_roll == 1:
...
#Hardcode for n > 1 qb
SYSTEM_SIZE = 1
ALL_ACTIONS = make_full_actions_list(QB1GATES, [], SYSTEM_SIZE)
...
POSSIBLE_TARGET_U = QB1GATES #[make_random_unitary(QB1GATES, [], nb_steps_per_gate, SYSTEM_SIZE) for _ in range(nb_wanted_unitaries)]
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None, randomise=True, poss_targets=POSSIBLE_TARGET_U):
        ...
        self.env = GateSynthesis(q1_gates=QB1GATES, q2_gates=[], full_action_list=ALL_ACTIONS ,
                                 rwd=100, max_steps=1000, init_uop=I, target_uop=poss_targets[idx], tol=1e-3)
