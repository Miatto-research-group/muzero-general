...
class MuZeroConfig:
    ...
    def __init__(self, nb_actions=9, observation_shape=(2, 2, 2), seed:int=42, nb_players:int=1, max_num_gpus=None):
        """
        ...
        self.num_simulations = 50  # Number of searches down the MCTS
        ...
        ### Training
        self.results_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../results", os.path.basename(__file__)[:-3], datetime.now().strftime("%Y-%m-%d--%H-%M-%S"))  # Path to store the model weights and TensorBoard logs
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = GateSynthesis(q1_gates=QB1GATES, q2_gates=[], rwd=100, max_steps=10,
                 init_uop=I, tol=1e-3)
...
class GateSynthesis:
    ...
    def __init__(self, q1_gates=[], q2_gates=[], rwd=100, max_steps=100,
                 init_uop=I, tol=1e-5):
    ...
    def ini_rd_gate(self):
        ...
        global GAMES_CPT
        GAMES_CPT += 1
        logger.info(f"Starting game number : {GAMES_CPT}")
        # decide on the depth of the target gate
        curr_depth = (GAMES_CPT // 10000) + 1  # increase depth every 10000 games
